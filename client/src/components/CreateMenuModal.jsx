...
const CreateMenuModal = ({ onClose, onCreate, menuType }) => {
  // Estado inicial diferenciado para menú diario y fijo
  const [formData, setFormData] = useState({
    name: "",
    description: "Almuerzo",
    ...(menuType === "daily" && {
      date: new Date(), // Guarda como Date object
      expiresAt: new Date(
    ...
    ...(menuType === "fixed" && {
      ...
      isImageMenu: false, // Bandera para saber si es un menú de imagen
      categories: [
        {
  ...
  const handleChange = (e) => {
    ...
    if (id === "date" || id === "expiresAt") {
      // Convierte el valor del input a Date object con compensación horaria
      ...
      setFormData({
        ...formData,
        [id]: adjustedDate, // Almacena como Date object
      });
    } else {
  ...
  const handleSubmit = (e) => {
    ...
    if (menuType === "daily") {
      if (formData.items.some((item) => !item.name.trim() || !item.category)) {
        toast.error("Todos los items deben tener nombre, precio y categoría");
        return;
      }
    }
  ...
  if (menuType === "fixed" && menuMode === "image") {
    if (!formData.imageUrl) {
      toast.error("Debes subir una imagen para el menú");
      return;
    }
    
    // Estructura para menú de imagen
    const menuData = {
      ...formData,
      categories: [], // No necesita categorías
      items: []       // No necesita items
    };
    
    onCreate(menuType, menuData);
    return;
  }

    if (menuType === "fixed" && menuMode === "form") {
      const hasEmptyItems = formData.categories.some((category) =>
        category.items.some(
